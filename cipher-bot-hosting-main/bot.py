...
from firebase_sync import (
    sync_payment_methods_to_firebase,
...
TOKEN = (
    ...
    or "8975432264:AAH6xBY7QEB8IYxoCyW_0iAsVFCC0DQtVdk"
).strip()
...
BRAND       = os.environ.get("BRAND_NAME") or os.environ.get("BOOT_NAME") or os.environ.get("BOT_NAME") or "⟦𝗖𝗢𝗗𝗜𝗡𝗚_𝙃𝙊𝙎𝙏𝙄𝙉𝙂⟧"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = os.environ.get("SUPPORT_USR") or os.environ.get("ADMIN_HANDLE") or "@CODINGJAMES_X"
UPDATE_CH   = os.environ.get("UPDATE_CH") or "https://t.me/CODINGJAMES_X"
...
try:
    OWNER_ID = int(os.getenv("OWNER_ID", "7831629041"))
except (TypeError, ValueError):
    OWNER_ID = 7831629041
...
PAYMENT_METHODS: Dict[str, Dict[str, Any]] = {
    "bkash":       {"name": "bKash",       "number": "", "type": "Send Money",       "tag": "[B]"},
    "nagad":       {"name": "Nagad",       "number": "", "type": "Send Money",       "tag": "[N]"},
    "rocket":      {"name": "Rocket",      "number": "", "type": "Send Money",       "tag": "[R]"},
    "upay":        {"name": "Upay",        "number": "", "type": "Send Money",       "tag": "[U]"},
    "binance":     {"name": "Binance Pay", "number": "", "type": "USDT (BEP20/TRC20)","tag": "[BP]"},
    "trustwallet": {"name": "Trust Wallet","number": "", "type": "USDT (BEP20/TRC20)","tag": "[TW]"},
    "bank":        {"name": "Bank",        "number": "", "type": "Bank Transfer",    "tag": "[BK]"},
}
...
def _persist_payment_methods() -> None:
    ...
    try:
        sync_payment_methods_to_firebase(PAYMENT_METHODS)
    except Exception as _e:
        print(f"[firebase] payment sync error: {_e}", flush=True)
    ...
    return jsonify(
        {
            ...
            "firebase_project": FIREBASE_CONFIG.get("projectId")
        }
    )


@_ka.route("/api/firebase-status")
def _ka_firebase_status() -> Any:
    conn = test_firebase_connection()
    return jsonify({
...
@_ka.route("/api/payment-methods", methods=["GET", "POST"])
def _ka_payment_methods() -> Any:
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        key = str(data.get("key", "")).strip().lower()
        if not key or key not in PAYMENT_METHODS:
            return jsonify({"ok": False, "error": "Invalid method key"}), 400
        new_number = str(data.get("number", "")).strip()
        PAYMENT_METHODS[key]["number"] = new_number
        _persist_payment_methods()
...
def render_adm_pay_methods(call: types.CallbackQuery) -> None:
    ...
    for key, m in PAYMENT_METHODS.items():
        ...
        num_str = esc(m['number']) if m.get('number') else "⚠️ Not configured"
        rows.append(f"{'✅' if enabled else '❌'} <b>{esc(m['name'])}</b> — "
                    f"<code>{num_str}</code> ({esc(m['type'])})")
    cap = (
...
def render_adm_pay_method_edit(call: types.CallbackQuery, key: str) -> None:
    ...
    cap = (
        ...
        f"{bullet('Number/Address',  esc(m['number']) if m.get('number') else '⚠️ Not configured (Tap button below to set)')}\n"
...
def render_payment_screen(call: types.CallbackQuery, data: str) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Number', esc(pm['number']) if pm.get('number') else '⚠️ Not configured yet (Contact Admin @CODINGJAMES_X)')}\n"
...
def main() -> int:
    ...
    # Firebase background sync & initial telemetry
    def _firebase_telemetry_loop() -> None:
        while True:
            try:
                time.sleep(60)
                _db = db_load()
    ...
    try:
        ...
        print(f"[firebase] init sync: {_fe}", flush=True)

    threading.Thread(target=_firebase_telemetry_loop, daemon=True, name="firebase-telemetry").start()
